...
void clear(int x)
{
   ...
   6:	c7 44 24 04 06 07 00 	movl   $0x706,0x4(%esp)
   d:	00 
   e:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  15:	e8 86 03 00 00       	call   3a0 <printf>
    exit();
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 339:	0f b6 92 19 07 00 00 	movzbl 0x719(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 37a:	e8 e3 fe ff ff       	call   262 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 3e6:	e8 77 fe ff ff       	call   262 <write>
...
putc(int fd, char c)
{
 ...
 456:	e8 07 fe ff ff       	call   262 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 473:	e8 ea fd ff ff       	call   262 <write>
  int c, i, state;
  uint *ap;
 ...
 4cf:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 4d1:	b8 12 07 00 00       	mov    $0x712,%eax
...
putc(int fd, char c)
{
 ...
 530:	e8 2d fd ff ff       	call   262 <write>
...
putc(int fd, char c)
{
 ...
 580:	e8 dd fc ff ff       	call   262 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 68a:	e8 3b fc ff ff       	call   2ca <sbrk>
  if(p == (char*)-1)
