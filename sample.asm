...
_sample:     file format elf32-i386
...
main(int argc, char *argv[])
{
  ...
  double z=0.0;
  fork();
   6:	e8 3f 02 00 00       	call   24a <fork>
  wait();
   b:	e8 4a 02 00 00       	call   25a <wait>
  for ( z = 0; z < 4000000.0; z += 0.01 )
  10:	d9 ee                	fldz   
  12:	dd 05 18 07 00 00    	fldl   0x718
  18:	dc c1                	fadd   %st,%st(1)
  1a:	d9 05 20 07 00 00    	flds   0x720
  20:	df ea                	fucomip %st(2),%st
  22:	77 f4                	ja     18 <main+0x18>
  24:	dd d8                	fstp   %st(0)
  26:	dd d8                	fstp   %st(0)
         x =  x + 3.14 * 89.64;
  exit();
  28:	e8 25 02 00 00       	call   252 <exit>
  2d:	66 90                	xchg   %ax,%ax
  2f:	90                   	nop
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 349:	0f b6 92 2b 07 00 00 	movzbl 0x72b(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 4e1:	b8 24 07 00 00       	mov    $0x724,%eax
